...
class BrowserNonnegativeRealSemiring {
...
test("browser JavaScript semirings execute refinements, batches, laws, and disposal", () => {
  const provider = new BrowserNonnegativeRealSemiring();
...
test("browser semiring context identity, optional bytes, and reentrancy are contained", () => {
  const provider = new BrowserNonnegativeRealSemiring();
  const first = llingLlang.semiring(provider, { domainId: "example.real.sum" });
  const second = llingLlang.semiring(
    new BrowserNonnegativeRealSemiring(), { domainId: "example.real.sum" },
  );
  ...
  const unstable = llingLlang.semiring(bare, { domainId: "example.real.raw" });
